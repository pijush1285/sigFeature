...
PlotErrors <- function(featsweepSigFe, ylim.min=0, ylim.max = 0){
  ...
  SigFe=sapply(featsweepSigFe, function(x) ifelse(is.null(x), NA, x$error))
  SigFe_1<-sort(SigFe, decreasing = TRUE)

  #For plotting the SD
  sigFeSD=sapply(featsweepSigFe, function(x) ifelse(is.null(x), NA, x$errorSD))
  sigFe_2<-sort(sigFeSD, decreasing = TRUE)

  #Plot easily.
  ym=1:length(featsweepSigFe)

  par(mfrow=c(1,2))
  plot(ym, SigFe_1, type="l", col="green", lwd=2, xlab="Number of Features", ylab="Mean external CV error", ylim=c(ylim.min, ylim.max))
  legend("topright", legend=c("sigFeature" ), col=c("green"), lwd=1)
  grid(nx=NULL, ny=NULL, col="gray", lty="dotted")

  plot(ym, sigFe_2, type="l", col="blue", lwd=2, xlab="Number of Features", ylab="Standard Deviation", ylim=c(ylim.min, ylim.max))
