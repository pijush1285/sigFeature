...
predict.sigFeature <- function(object, newdata, decision.values=FALSE,
    ...
    if(inherits(newdata, "simple_triplet_matrix")){
        ...
        stop("test data does not match model !")
            ret <- .C("svmpredict", as.integer(decision.values),
            ...
            as.integer(object$labels), as.integer(object$nSV),
            as.integer(object$sparse), as.integer(object$type),
            as.integer(object$kernel), as.integer(object$degree),
            as.double(object$gamma), as.double(object$coef0),
        ...
        ret = double(nrow(newdata)), dec = double(nrow(newdata) *
                object$nclasses * (object$nclasses - 1)/2),
        prob = double(nrow(newdata) * object$nclasses), PACKAGE = "e1071")
    ...
    if(decision.values) {
    colns = c()
    for(i in seq_len(object$nclasses - 1))
        for(j in (i + 1):object$nclasses) colns <- c(colns,
        ...
        probabilities not available for predictions.")
