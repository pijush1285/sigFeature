...
.sigFeature.enfold <- function(test.fold, X, Y) {
  ...
  return(list(feature.ids=features.ranked, train.data.ids=row.names(train.data),
...
sigFeature.enfold <- function(x,y,CV, CVnumber=0){
  if(CV=="kfold"){
    ...
    nrows = nrow(x)
    folds = rep(1:CVnumber, len=nrows)[sample(nrows)]
    folds = lapply(1:CVnumber, function(x) which(folds == x))
    results = lapply(folds, .sigFeature.enfold, x, y)
  }else{
    print("LOO cross validation selected")
    nrows = nrow(x)
    results = lapply(1:nrows, .sigFeature.enfold, x, y)
  }
  return(results)
}
