...
sigFeature = function(X, Y){
  ...
  pvals <- lapply(1:dim(clsA)[2], function(i)
    t.test(clsA[ ,i],clsB[ ,i])$p.value)
  md    <- unlist(pvals)
  y = c(rep(-1, as.vector(table(Y))[1]),rep(1, as.vector(table(Y))[2]))
  x <- X[c(m[which(Y==names(table(Y))[1])],m[which(Y==names(table(Y))[2])]), ]
  n = ncol(x)
  breatheFeaturesIndexes = seq(1:n)
  ...
  while(length(breatheFeaturesIndexes)>0){
    ...
    featureCodifiedList[codifiedFeatureIndex] = breatheFeaturesIndexes[ranking[1]]
