---
title: 'sigFeature: an R-package for significant feature selection using SVM-RFE & t-statistic'
...
vignette: >
  %\VignetteIndexEntry{sigFeature: Significant Feature Selection using SVM RFE & t-statistic}
...
```{r}
mytitle<-'Box Plot'
boxplot(unlist(pvalsigFe), unlist(pvalRFE), main=mytitle, names=c("sigFeature", "SVM-RFE"),
...
pheatmap(x[ ,sigfeatureRankedList[1:20]], scale="row", clustering_distance_rows="correlation")
```
...
```{r}
pheatmap(x[ ,featureRankedList[1:20]], scale="row", clustering_distance_rows="correlation")
```
...
#To run the code given bellow will take huge time. Thus the process data is given below. 
